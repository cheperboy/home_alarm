...
class ThreadExtAlarmGateway(Thread):
    ...
    def __init__(self, socketio):
        ...
        self.PUB_COMMAND.bind("tcp://*:%s" % zmq_socket_config.port_socket_extalarm_command)        
        ...
        self.SUB_STATE.connect ("tcp://localhost:%s" % zmq_socket_config.port_socket_extalarm_state)
        ...
        super(ThreadExtAlarmGateway, self).__init__()
    ...
    def run(self):
        ...
        Forwards commands (start/stop alarm) from Flask app to Alarm Process
        """
        prctl.set_name("ExtGateway")  # set thread name visible in htop
    ...
    def forward_command_from_web_to_alarm(self):
        ...
        if self.command_alarm is not None:
            if self.command_alarm is True:
                ...
                logger.debug('Extalarm gateway forwad command Start')
            if self.command_alarm is False:
                ...
                logger.debug('Extalarm gateway forwad command Stop')
    ...
    def forward_request_status_from_web_to_alarm(self):
        ...
        if self.event_request_status is True:
            ...
            logger.debug('Extalarm gateway forward request status update')
