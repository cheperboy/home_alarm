...
class DBLog(db.Model):
    ...
    scope   = db.Column(db.String(20))  # ext/nox
    ...
    def __repr__(self):
        return '<DBLog {0} {1} {2} {3} {4} {5} {6} {7} {8}>'.format(self.id, self.dt, self.ip, self.user, 
                                                                        self.scope, self.badge, self.subject, 
    ...
    @classmethod
    def log_nox_bind_ext(self, limit=None):
        """ All logs about nox and about detection (including those related to ext)"""
        query = db.session.query(DBLog) \
                .filter(or_(DBLog.scope == "nox", DBLog.message == "detection")) \
                .order_by(DBLog.id.desc())
        if limit:
            query = query.limit(limit)
        return(query.all())
